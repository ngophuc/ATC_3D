...
main( int argc, char** argv )
{
  ...
  //args.addOption( "-drawContourSRC", "-drawContourSRC <color> <linewidth> <depth>: draw the original source contour.", "10", "30", "40" );
  //args.addOption( "-drawCntPointSRC", "-drawCntPointSRC <color> <linewidth> <depth>: draw the original source contour.", "10", "30", "40" );
  ...
  //args.addOption("-displaySmoothContour", "-displaySmoothContour <filename> <color> display the smoothed contour with the noise constraints", "", "0" );
  ...
  */
  ...
  if(args.check("-displaySmoothContour")){
      ...
      string nameSmoothContour = args.getOption("-displaySmoothContour")->getValue(0);
      //cout<< nameSmoothContour <<endl;
      ofstream outSmoothContour;
      outSmoothContour.open( nameSmoothContour.c_str() , ofstream::out );
      outSmoothContour << "# SmoothContour exported from displayNoiseBS (source ImaGene): " << std::endl;
      outSmoothContour << "# Format X Y " << std::endl;
      //for(vector<Vector2D>::const_iterator it = contourSmooth.begin(); it!= contourSmooth.end(); it++)
      for(int i=0; i< contourSmooth.size(); i++){
         Vector2D ptA = contourSmooth.at(i);
         outSmoothContour << ptA.x() << " " << ptA.y() << std::endl;
      }
      outSmoothContour.close();

      long time = Clock::stopClock();
      cerr << "Noise removed  in " << time << " ms."<<  endl;
      uint color = args.getOption("-displaySmoothContour")->getIntValue(1);
      if(color > 0)
